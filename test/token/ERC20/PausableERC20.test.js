...
contract('PausableERC20', function ([owner, other, thirdParty]) {
  ...
  context('creating valid token', function () {
    describe('as a PausableERC20', function () {
      describe('without initial supply', function () {
        it('should fail', async function () {
          await expectRevert(
            ...
            'PausableERC20: supply cannot be zero',
          );
        });
      });
      ...
      describe('with initial supply', function () {
        beforeEach(async function () {
          this.token = await PausableERC20.new(
            _name,
            _symbol,
            _decimals,
            _initialSupply,
  ...
  context('PausableERC20 token behaviours', function () {
    beforeEach(async function () {
      this.token = await PausableERC20.new(
        _name,
        _symbol,
    ...
    context('like a PausableERC20', function () {
      describe('when the sender doesn\'t have pauser permission', function () {
        const from = thirdParty;

        it('cannot pause', async function () {
          await expectRevert(
            this.token.pause({ from }),
